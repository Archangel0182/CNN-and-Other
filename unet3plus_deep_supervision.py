...
def unet3plus(input_shape, num_classes=1, deep_sup=True, dropout=0.3, batchnorm=True):
    """ Inputs """
    ...
    """ Deep Supervision """
    if deep_sup == True:
        y1 = layers.Conv2D(num_classes, kernel_size=3, padding="same")(d1)
        y1 = layers.Activation("sigmoid")(y1)

        y2 = layers.Conv2D(num_classes, kernel_size=3, padding="same")(d2)
        y2 = layers.UpSampling2D((2, 2), interpolation="bilinear")(y2)
        y2 = layers.Activation("sigmoid")(y2)

        y3 = layers.Conv2D(num_classes, kernel_size=1, padding="same")(d3)
        y3 = layers.UpSampling2D((4, 4), interpolation="bilinear")(y3)
        y3 = layers.Activation("sigmoid")(y3)

        y4 = layers.Conv2D(num_classes, kernel_size=1, padding="same")(d4)
        y4 = layers.UpSampling2D((8, 8), interpolation="bilinear")(y4)
        y4 = layers.Activation("sigmoid")(y4)

        y5 = layers.Conv2D(num_classes, kernel_size=1, padding="same")(e5)
        y5 = layers.UpSampling2D((16, 16), interpolation="bilinear")(y5)
        y5 = layers.Activation("sigmoid")(y5)
